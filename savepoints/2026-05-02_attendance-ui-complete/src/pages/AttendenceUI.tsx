...
const TeacherTicker: React.FC<{ barBg?: string; panelBg?: string; barIcon?: string }> = ({ barBg, panelBg, barIcon }) => {
  const [open, setOpen] = useState(false)
  const tickerText = mockAttendance.notices.map(n => `${n.author}: ${n.message}`).join('   ·   ')
            ...
            <motion.div
              ...
              style={{
                ...(panelBg
                  ...
                  : { background: 'rgba(8,4,0,0.97)', border: '1px solid rgba(255,215,0,0.35)' }),
                ...
                {mockAttendance.notices.map((n) => (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: '1rem' }}>
                      <span style={{ fontSize: '1.1rem', letterSpacing: '1px', color: 'rgba(255,215,0,0.9)' }}>{n.author}</span>
                      <span style={{ fontSize: '0.8rem', color: 'rgba(255,215,0,0.3)', flexShrink: 0 }}>{n.time}</span>
                    </div>
                    <span style={{ fontSize: '0.8rem', letterSpacing: '1px', color: 'rgba(255,215,0,0.4)' }}>{n.role}</span>
                    <p style={{ margin: '0.25rem 0 0', fontSize: '1rem', letterSpacing: '0.5px', color: 'rgba(255,215,0,0.75)', lineHeight: 1.5 }}>
                      {n.message}
                    </p>
                  </div>
                ))}
        ...
        >
          <div>
            <p style={{ ...VT, margin: 0, fontSize: '1.4rem', letterSpacing: '2px', color: checkedIn ? 'rgba(100,220,100,0.9)' : 'rgba(255,215,0,0.9)' }}>
              {checkedIn ? '✓ REGISTER SIGNED' : 'SIGN THE REGISTER'}
            </p>
