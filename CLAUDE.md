...
- Custom themes (via Theme Adder) use a simplified asset set: banner, button, music, 7 page backgrounds, and 2 sprite types (home moving/clickable + attendance static). Login page is fixed (not theme-customisable). No frame_main, input_box, or ambient_sprite — custom themes use `--color-pane-bg` (opaque solid) for panels instead. Assets are grouped per UI page (background + alt bg + sprites together).
- Shop layout is theme-first: learner selects a theme, then browses category sub-tabs. BACKGROUNDS tab shows non-purchasable page background previews (BgPageCard). ALT BACKGROUNDS tab shows purchasable alternates. Finalize endpoint auto-creates all sub-items (alt bgs, sprites, soundtrack) when a theme is finalized.
